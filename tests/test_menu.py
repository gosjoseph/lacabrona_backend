...
def test_create_then_get_item(api_client):
    client, _ = api_client
    create = client.post("/api/menu", json=_payload())
    assert create.status_code == 201
    fetch = client.get("/api/menu/taco-1")
...
def test_create_duplicate_returns_409(api_client):
    client, _ = api_client
    client.post("/api/menu", json=_payload())
    second = client.post("/api/menu", json=_payload())
    assert second.status_code == 409


def test_get_missing_returns_404(api_client):
    client, _ = api_client
    assert client.get("/api/menu/none").status_code == 404


def test_filter_by_category(api_client):
    client, _ = api_client
    client.post("/api/menu", json=_payload(id="t1", category="tacos"))
    client.post("/api/menu", json=_payload(id="b1", category="bebidas"))

    response = client.get("/api/menu", params={"category": "tacos"})
...
def test_update_partial_fields(api_client):
    client, _ = api_client
    client.post("/api/menu", json=_payload())
    response = client.put("/api/menu/taco-1", json={"price": 250.0})
    assert response.status_code == 200
...
def test_update_with_no_fields_returns_400(api_client):
    client, _ = api_client
    client.post("/api/menu", json=_payload())
    response = client.put("/api/menu/taco-1", json={})
    assert response.status_code == 400


def test_update_missing_returns_404(api_client):
    client, _ = api_client
    response = client.put("/api/menu/none", json={"price": 1.0})
...
def test_delete_existing_returns_204(api_client):
    client, db = api_client
    client.post("/api/menu", json=_payload())
    response = client.delete("/api/menu/taco-1")
...
def test_delete_missing_returns_404(api_client):
    client, _ = api_client
    assert client.delete("/api/menu/none").status_code == 404
