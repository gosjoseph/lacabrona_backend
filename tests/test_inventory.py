...
def test_update_partial(api_client):
    client, _ = api_client
    client.post("/api/v1/inventory", json=_payload())
    response = client.put("/api/v1/inventory/tomate", json={"supplier": "Nueva Granja"})
    assert response.status_code == 200
    body = response.json()
...
def test_update_missing_returns_404(api_client):
    client, _ = api_client
    assert client.put("/api/v1/inventory/none", json={"stock": 1.0}).status_code == 404


def test_adjust_stock_positive_delta(api_client):
    client, _ = api_client
    client.post("/api/v1/inventory", json=_payload(stock=10.0))
    response = client.post("/api/v1/inventory/tomate/adjust", json={"delta": 3.5})
    assert response.status_code == 200
...
def test_adjust_stock_clamped_at_zero(api_client):
    client, _ = api_client
    client.post("/api/v1/inventory", json=_payload(stock=2.0))
    response = client.post("/api/v1/inventory/tomate/adjust", json={"delta": -10.0})
    assert response.status_code == 200
...
def test_adjust_missing_returns_404(api_client):
    client, _ = api_client
    response = client.post("/api/v1/inventory/none/adjust", json={"delta": 1.0})
