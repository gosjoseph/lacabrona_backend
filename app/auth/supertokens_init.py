...
async def _attach_internal_claims(response: Any) -> None:
    """Look up the signed-in email, persist supertokens_user_id, merge claims.

    Rejects the signin (by raising) if the email belongs to neither collection.
    """
    ...
    db = get_db()
    try:
        result = resolve_user_type(email or "", supertokens_user_id, db)
